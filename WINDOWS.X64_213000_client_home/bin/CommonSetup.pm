# 
# $Header: install/utl/scripts/db/CommonSetup.pm.pp /st_install_21.3/1 2021/08/11 01:48:37 poosrini Exp $
#
# CommonSetup.pm
# 
...
#
#    MODIFIED   (MM/DD/YY)
#    poosrini    08/09/21 - bug 33037152: create scratchPath
#    poosrini    02/02/21 - Revert WA for bug-31718182
#    poosrini    09/23/20 - XbranchMerge poosrini_bug-31718182 from main
#    davjimen    07/23/20 - DICDI-4574 set logs and checkpoints dir for pilot call
#    pkuruvad    07/14/20 - RTI 22847161 - set scratchpath outside OH
#    josepe      05/12/20 - BUG 31323484 replacing use of id with whoami
#    poosrini    05/01/20 - DICDI-4474 (JDEVADF refresh)
#    poosrini    03/05/20 - Bug 30992807: Add osdt_core.jar to classpath
#    davjimen    02/11/20 - remove jsch jar from the classpath
#    poosrini    01/27/20 - sshd/slf4j jars
#    hrangasw    09/06/19 - Bug: 30258828 - classpath issue fix
#    hrangasw    07/12/19 - NextGen
#    josepe      06/24/19 - BUG_29057109 adding ahf jar to the classpath.
#    davjimen    05/31/19 - include hyphen as possible char in the username
#    josepe      05/22/19 - BUG_29721705 Using oh/jdk instead of /oh/jdk/jre as
#                           jvm.
#    josepe      05/06/19 - BUG_29721499 adding a new clause to the rootprechk
#                           execution
#    vansoni     03/04/19 - adding jackson jars under jlib dir to classpath
#    josepe      01/31/19 - BUG_29229841 Polishing way to present invalid cmds
#                           for patching
#    josepe      11/15/18 - BUG_28911311 Changing current directory before
#                           perform find command.
#    josepe      10/18/18 - Adding addImageInfo parameter.
...
# To get the user executing the script
my $WHOAMI = exists($ENV{WHOAMI}) ? $ENV{WHOAMI} : '/usr/bin/whoami';
...
# Flag to determine if the -silent flag was used
my $isSilent=0;

# Determine if the user already run rootpre.sh
my $rootPreExecuted="n";
...
my $isSilentArg="-silent";
my $rootPreExecutedArg="-rootPreExecuted";
...
# parameters for adding info to the gold image
my $addImageInfoArg = "-addImageInfo";
my $addImageInfo = 'false';
my $addImageInfoString = ""; 
...
sub main {
	...
	$scratchPath = $self->calculateScratchPath();

	# Create the scratch path directory
	$self->createScratchPathDirectory();
	
	# Get the JRE location
	$JRELOC = $ORACLE_HOME.$dirSep.'jdk';
...
sub launchPilot() {
	...
	my $javaCmd = $self->getJavaCmd($PLATFORM, $oracleHome.$dirSep.'jdk', $dirSep, $PLATFORM_DIRECTORY_NAME);
	my $classPath = $self->getClassPath();

	# Include pilot specific jars into the classpath
        my @pilotJars;
        push(@pilotJars, $oracleHome.$dirSep.'install'.$dirSep.'jlib'.$dirSep.'dblcm.jar');
        $classPath = $classPath.$pathSep.join($pathSep, @pilotJars);

	# Determine base location
	my $pluginBaseSuffix = 'inst_plugin'.$dirSep.'base';
	# Default to OH/install/inst_plugin/base
	my $homeBaseLoc = $oracleHome.$dirSep.'install'.$dirSep.$pluginBaseSuffix;
        my $baseLoc = $homeBaseLoc;
	my $orabase = 'orabase';
	if($isWindows) {
		$orabase = 'orabase.exe';
	}
	my $orabaseCmd = $oracleHome.$dirSep.'bin'.$dirSep.$orabase;
	my $orabaseLoc = `$orabaseCmd`;
	if($? == 0) {
		chomp($orabaseLoc);
		if(-w "$orabaseLoc") {
			# OH/cfgtoollogs/inst_plugin/base
			$baseLoc = "$orabaseLoc".$dirSep.'cfgtoollogs'.$dirSep.$pluginBaseSuffix;
		}
	}
	print("Plugin base: $baseLoc\n");
        # Logs directory inside the base location
        my $logDir = "$baseLoc".$dirSep."logs";
	...
	push(@cmd, "-cp");
	push(@cmd, $classPath);
	push(@cmd, "-Dprism.order=sw");
	push(@cmd, "-Xms150m");
	push(@cmd, "-Xmx2048m");
	push(@cmd, "oracle.install.commons.pilot.PilotApplication");
	push(@cmd, "PILOT_HOME=$oracleHome");
	push(@cmd, "PILOT_BASE=$homeBaseLoc");
	push(@cmd, "PILOT_RUN_MODE=ORACLE_HOME_SELF_EXTRACTED");
	push(@cmd, "PILOT_ORACLE_HOME=$oracleHome");
	push(@cmd, "-logDir");
	push(@cmd, "$logDir");
	push(@cmd, "-checkpointDir");
	push(@cmd, "$baseLoc");

	# Filter the global arguments
	my @args;
	foreach my $arg (@ARGV) {
		if(!($arg =~ /CVU_OS_SETTINGS/)) {
			push(@args, $arg);
		}
	}

	# Execute the pilot command
	my $pilotCmd = join(' ',@cmd).' '.join(' ',@args);
...
sub calculateScratchPath {
	return $tempLogDir.$dirSep.$scratchPathSuffix;
}

sub createScratchPathDirectory() {
	File::Path::make_path($scratchPath) or die "mkdir failed: $!";
}
...
sub rootAndAdminUserCheck() {
	...
	if (! $isWindows) {
		# BUG_31323484 removing ID usage, replacing with whoami
		$user = `$WHOAMI`;
...
sub rootPreCheck() {
	...
	if(! $isWindows ) {
		...
		# BUG_29721499 - if user uses -rootPreExecuted flag we emulate the behaviour of oracle.install.skipRootPre=true
		if (lc($rootPreExecuted) eq "y"){
			push(@ARGV, 'oracle.install.skipRootPre=true');
		}
...
sub rootPreMsg() {
	# Ask the user for the flag oracle.install.skipRootPre=true
	print "Please ensure that rootpre.sh has been run by root in this machine, after that, ";
	print "you can use the property -rootPreExecuted Y to indicate that you already run rootpre.sh.\n";
	exit 1;
}
...
sub getClassPath() {
	...
	$ORACLE_HOME.$dirSep.'jlib'.$dirSep.'jackson-annotations.jar',
	$ORACLE_HOME.$dirSep.'jlib'.$dirSep.'jackson-core.jar',
	$ORACLE_HOME.$dirSep.'jlib'.$dirSep.'jackson-databind.jar',
	$ORACLE_HOME.$dirSep.'jlib'.$dirSep.'entityManager_proxy.jar',
	$ORACLE_HOME.$dirSep.'jlib'.$dirSep.'javax.security.jacc_1.0.0.0_1-1.jar',
	$ORACLE_HOME.$dirSep.'jlib'.$dirSep.'orai18n-collation.jar',
	...
	$ORACLE_HOME.$dirSep.'jlib'.$dirSep.'AHFInstall.jar',
	$ORACLE_HOME.$dirSep.'jlib'.$dirSep.'slf4j-api.jar',
	$ORACLE_HOME.$dirSep.'jlib'.$dirSep.'slf4j-jdk14.jar',
	$ORACLE_HOME.$dirSep.'jlib'.$dirSep.'sshd-cli.jar',
	$ORACLE_HOME.$dirSep.'jlib'.$dirSep.'sshd-common.jar',
	$ORACLE_HOME.$dirSep.'jlib'.$dirSep.'sshd-mina.jar',
	$ORACLE_HOME.$dirSep.'jlib'.$dirSep.'sshd-core.jar',
	$ORACLE_HOME.$dirSep.'jlib'.$dirSep.'sshd-sftp.jar',
	$ORACLE_HOME.$dirSep.'jlib'.$dirSep.'osdt_core.jar',
        $ORACLE_HOME.$dirSep.'jlib'.$dirSep.'ewt3.jar',
        $ORACLE_HOME.$dirSep.'jlib'.$dirSep.'help-share.jar',
        $ORACLE_HOME.$dirSep.'jlib'.$dirSep.'jewt4.jar',
        $ORACLE_HOME.$dirSep.'jlib'.$dirSep.'ohj.jar',
        $ORACLE_HOME.$dirSep.'jlib'.$dirSep.'olaf2.jar',
        $ORACLE_HOME.$dirSep.'jlib'.$dirSep.'oracle_ice.jar',
        $ORACLE_HOME.$dirSep.'jlib'.$dirSep.'share.jar',
        $ORACLE_HOME.$dirSep.'jlib'.$dirSep.'ojmisc.jar',
        $ORACLE_HOME.$dirSep.'lib'.$dirSep.'xml.jar',
        $ORACLE_HOME.$dirSep.'lib'.$dirSep.'xmlparserv2.jar',
	...
	# include all jars of oui in classpath
	
	#If NextGen
	if (-e $ORACLE_HOME.$dirSep.'inventory'.$dirSep.'registry.xml')
	{
	    $self->addJarToClassPath($ORACLE_HOME.$dirSep.'oui'.$dirSep.'modules'.$dirSep.'OraInstaller.jar', \@INSTALL_JARS);
	}
	else
	{
...
sub parseArgs() {
	...
	for(my $count = 0; $count < $javaOptionsSize; $count++) {
		...
		if($firstTwoChars eq "-J") {
		...
		} elsif(lc($javaOption) eq lc($pluginArg) or lc($javaOption) eq lc($setupOrdsArg)) {
			$launchPilot="true";
			last;
			...
			if($applyingPSU eq 'true') {
				# BUG_29229841 We are already patching, checking if is the same patch
				if($javaOption eq $currentPatch){
					$self->repetitiveArgsError($currentPatch);
				} else {
					$self->doublePatchingError($currentPatch, $javaOption);
				}
			}
			push(@patchArgs, $javaOption);
			$currentPatch = $javaOption;
		...
		} elsif(lc($javaOption) eq lc($addImageInfoArg)){
			$addImageInfo = 'true';
			if($count+1 < $javaOptionsSize and substr($ARGV[$count+1],0,1) ne "-"){
				$count++;
				$addImageInfoString = $ARGV[$count];
			}	
		} elsif(lc($javaOption) eq lc($isSilentArg)){
			$isSilent = 1;
			push(@remainingArgs, $javaOption);
		} elsif(lc($javaOption) eq lc($rootPreExecutedArg)){
			if($count+1 < $javaOptionsSize and lc($ARGV[$count+1]) eq "y"){
				$count++;
				$rootPreExecuted = $ARGV[$count];
			} else {
				$self->rootPreMsg();
			}
		} else {
...
sub getImageInfo() {
	...
	if($printImageInfo eq 'true') {
                my $imageInfoCmd = $JAVACMD.' -cp '.$classPath.$javadebugOpts.' oracle.install.library.util.imageinfo.ImageInfoUtil '.$ORACLE_HOME_ARG.' '.$ORACLE_HOME.' '.$showAdditionalInfoArg.' '.$showAdditionalInfo;
...
# routine to add additional info to the goldimage
sub addAdditionalImageInfo(){
	my ($self, @args) = @_;
	#Checking if its needed to add info to the gold image
	if($addImageInfo eq 'true') {
                my $addImageInfoCmd = $JAVACMD.' -cp '.$classPath.$javadebugOpts.' oracle.install.library.util.imageinfo.ImageInfoUtil '.$ORACLE_HOME_ARG.' '.$ORACLE_HOME.' '.$addImageInfoArg.' '.$addImageInfoString;
		my $addImageInfoOut = qx{$addImageInfoCmd};
		my $checkAddImageInfoStatusCode = $?;
		if ($addImageInfoOut ne "") {
			print "\n$addImageInfoOut\n";	
		}
		$self->terminate($checkAddImageInfoStatusCode);
	}
}
...
###
# To show proper error message when two patches are selected,
# and are not supported simultaneously.
###
sub doublePatchingError() {
        my $self = shift;
	my $firstPatch = shift;
	my $secondPatch = shift;
	
	print "\nERROR: Arguments $firstPatch and $secondPatch cannot be used simultaneously.\n";
	exit 1;
}
...
sub bootstrap() {
	...
	if($isWindows) {
		...
		push(@dllFiles, $homeBin.$dirSep."orasrvm.dll");
		push(@dllFiles, $homeBin.$dirSep."orawsec.dll");
	...
	my $jreDir = $tempHome.$dirSep."jdk";
	if($customJRE eq 'false') {  
		File::Path::make_path($jreDir);
		$self->copyAllFiles($ORACLE_HOME.$dirSep."jdk", $jreDir);
	} else {
		$jreDir = $JRELOC;
	}
	...
	if (-e $ORACLE_HOME.$dirSep.'inventory'.$dirSep.'registry.xml')
	{
		my $ouiJlibJarsPath = $ORACLE_HOME.$dirSep."oui".$dirSep."modules";
